...
#### 3.3.6 Content Plans (Recurring Content Scheduling)

**Requirements:**
- Users can create "Content Plans" - reusable configurations for recurring content generation
- Each plan defines: prompt template, content sources, schedule, and generation settings
- System maintains memory of all posts generated from each plan to avoid repetitions
- Plans execute automatically at scheduled times, generating unique content each time
- AI uses plan history to ensure variety and avoid duplicate topics/angles

**Content Plan Configuration:**
- **Name**: User-defined name for the plan (e.g., "Daily Tech Tips", "Weekly Market Summary")
- **Prompt Template**: The base prompt used for generation (can include variables like {date}, {day_of_week})
- **Content Sources** (optional): Selected scraped content sources to use as inspiration
- **Schedule**: Cron-style schedule or simple presets (daily at X, weekly on Y at Z, custom)
- **Generation Settings**:
  - Channel to publish to
  - Tone/style overrides (or use channel defaults)
  - Image generation: enabled/disabled, SVG vs raster, style settings
  - Publishing mode: auto-publish, review-first, or draft-only
- **Memory Settings**:
  - Lookback period for avoiding repetition (e.g., last 30 days, last 50 posts)
  - Topics/angles to explicitly avoid
  - Diversity requirements (e.g., "vary the topic category each day")

**Plan Memory & Repetition Avoidance:**
- System stores summary/keywords of each post generated from the plan
- Before generating new content, AI reviews recent plan history
- AI is instructed to cover different angles, topics, or aspects than previous posts
- User can view plan history and mark certain posts as "allow similar" if needed
- System tracks themes/topics covered and suggests underexplored areas

**Example Use Cases:**
1. **Daily Tips**: "Generate a daily tip about {channel_topic}" - system ensures each day's tip is unique
2. **Weekly Roundup**: "Summarize the week's news from sources" - system uses scraped content from that week
3. **Motivational Quotes**: "Generate an inspiring quote" - system avoids repeating similar sentiments
4. **Product Highlights**: "Feature a different product aspect" - system cycles through features

**Acceptance Criteria:**
- [ ] User can create, edit, and delete content plans
- [ ] User can configure prompt template with optional variables
- [ ] User can attach content sources to a plan
- [ ] User can set schedule using presets or custom cron expression
- [ ] User can choose publishing mode (auto-publish, review, draft)
- [ ] System executes plans automatically at scheduled times
- [ ] System maintains history of all posts generated from each plan
- [ ] AI receives plan history context to avoid repetition
- [ ] User can view plan execution history with generated posts
- [ ] User can pause/resume individual plans
- [ ] User can manually trigger plan execution ("generate now")
- [ ] System provides analytics: posts generated, success rate, topic diversity
- [ ] Plan execution respects channel's rate limits and posting windows
- [ ] Failed plan executions are logged and optionally retried
- [ ] User receives notifications for plan executions (configurable)
...
### 6.8 Content Plan Setup Flow

```
1. User navigates to channel -> "Content Plans"
2. User clicks "Create Plan"
3. User configures plan:
   a. Name: "Daily Tech Tips"
   b. Prompt template: "Generate a useful tip about {topic} for developers"
   c. Sources (optional): Select content sources for inspiration
   d. Schedule: Daily at 9:00 AM
   e. Publishing mode: Review-first
   f. Image settings: Enable SVG generation
4. User saves the plan
5. Plan appears in plan list with status "Active"
6. User can pause/resume/edit/delete plans
```

### 6.9 Content Plan Execution Flow

```
1. Scheduler triggers plan at configured time
2. System loads plan configuration and memory:
   - Fetches prompt template and settings
   - Loads recent posts generated by this plan (based on lookback period)
   - Extracts topics/themes already covered
3. System prepares generation context:
   - If sources configured: fetches recent scraped content
   - Builds "avoid" list from plan memory
   - Injects variables into prompt template ({date}, {day_of_week}, etc.)
4. AI generates content:
   - Receives full context including history
   - Instructed to create unique content not similar to recent posts
   - Generates text and optional image
5. System creates post:
   a. Auto-publish: Publishes immediately
   b. Review-first: Sends to bot for approval
   c. Draft-only: Saves as draft for manual review
6. System updates plan memory:
   - Stores summary/keywords of generated content
   - Logs execution status
7. User notified via bot (if enabled)
```
...
- **ContentPlan**: Recurring content generation configuration (name, prompt template, schedule, settings)
- **ContentPlanSource**: Junction table linking content plans to content sources
- **ContentPlanExecution**: Log of each plan execution (timestamp, status, generated post reference)
- **ContentPlanMemory**: Summary/keywords of generated posts for repetition avoidance
...
- **Content Plans** - Recurring content generation with memory:
  - Create reusable content generation configurations
  - Schedule recurring posts (daily, weekly, custom cron)
  - Plan memory to avoid repetition across executions
  - Support for prompt templates with variables
  - Integration with content sources for inspiration
  - Analytics on plan performance and topic diversity
